# [DBB] 规划 / 审查协作协议（可选，实验性）

用途：让 豆包 当「规划与审查大脑」，本地 agent 保留全部执行权。
与 deepseek-brain 的 `[DSB]`、codex-with-chatgpt 的 `[C2C]` 同构；
数据面是**推送**——豆包 网页版不能自己拉数据。
...
checkpoint 自动写入 session（`dbb thread status --json` 可查）。
...
- **PLAN**（豆包 → 你）：应含 ACTIONS / FILES_LIKELY_INVOLVED / TESTS / SUCCESS_CRITERIA；
  只有空泛结论就要求展开一次。
- **EXECUTED**（你 → 豆包）：只报元数据（改动文件数、测试结果）+「请复核」。
  执行权永远在本地，**不要**交出去。
- **REVIEW**（豆包）：DONE / PLAN（下一轮）/ BLOCKED。

## 限额与断点

- 迭代上限默认 12；到顶暂停问用户。
- 进度写进工作区 session（`dbb session set`）：`protocolState / waitingFor / nextStep / knownIssues`（有长度上限）。
- 会话丢失 → 依据 session 生成 HANDOFF 简报（目标 / 进度 / 当前状态 / 已知问题 / 下一步），**不粘贴日志或 diff**。

## 注意

- 本协议是**约定**：豆包 不会真的执行任何操作，执行永远由本地 agent 完成。
