...
- 人工登录一次后**通常**长期复用（字节系 cookie 是持久型，登录持久化比 Gemini 简单得多）；
  平台风控或服务端会话过期时仍需重新登录
...
豆包的能力栏在本族 brain 中覆盖较广（`dbb list-models` 可查当前可用项）：
...
node "$SKILL_ROOT/scripts/dbb/cli.mjs" ask \
  ...
  --capability "视频生成" --thread new --timeout 900000 --json
...
agent 认出后会自己去调 `dbb`（切能力、发提示词、等产物、下载到本地），你只管描述需求。

| 你想做什么 | 直接对 agent 说 |
| --- | --- |
| 生成图片 | 「让豆包画一只布偶猫趴在窗台上，油画风格」 |
| 生成视频 | 「用豆包做一段熊猫在竹林里啃竹子的短视频」 |
| 长文写作 | 「让豆包帮我写一篇项目介绍」 |
| 生成音乐 / 播客 | 「让豆包生成一段关于春天的音乐」「让豆包做个播客聊聊最近的技术新闻」 |
| 录音转写 | 「用豆包把这段录音转成文字」（把音频文件路径一并告诉 agent） |
| 分析文件 | 「让豆包分析一下这个文件讲了什么」 |

使用要点：

- **点名最稳**：话里带上「豆包」或「dbb」，agent 就会走本 skill；也支持英文触发
  （`use doubao` / `ask doubao` / `doubao image generation` / `doubao write` / `doubao analyze`）。
- **生成类要等**：生图约半分钟，**生视频约 3 分钟起**（站点可能提示 10 分钟），慢是正常的，别当成卡死。
- **默认自动确认参数与额度消耗**：豆包生成前会列参数并要求确认（视频必现），CLI 默认自动回复确认；
  想改成人工确认，就对 agent 说「这次我手动确认参数」
  （agent 会加 `--no-auto-confirm`，此时还需配合 `--keep-open` 保持窗口打开，否则来不及操作）。
- **产物会自动下载**：agent 会把本地绝对路径给你；平台水印无法去除。
- **首次要先配置**：需要先完成安装与首次登录（见 [安装](#安装) 章节）。
...
- **回答正文默认不落盘**，只记录元数据（例外：`--debug` 或失败时保存的 `debug/` 快照会含未脱敏正文，排障后请删除）；**产物文件**按需落盘
...
五个「网页版大脑」共享同一套机制层（净化闸门、会话、日志、CLI 契约），
