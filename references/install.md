...
git clone https://github.com/ops120/doubao-brain ~/.agents/skills/doubao-brain     # 通用
```
...
依次：检查 Node 与浏览器 → 把依赖装到状态目录 → 打开浏览器**请用户登录豆包** → 导出登录态。
...
- Windows `%LOCALAPPDATA%\doubao-brain\`；macOS `~/Library/Application Support/doubao-brain/`；Linux `$XDG_STATE_HOME/doubao-brain/`（`DBB_STATE_DIR` 可覆盖）。
