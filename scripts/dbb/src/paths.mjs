...
export function stateDir() {
  const override = process.env.DBB_STATE_DIR;
  if (override && override.trim()) return path.resolve(override.trim());
  const home = os.homedir();
  if (process.platform === "win32") {
    const base = process.env.LOCALAPPDATA || path.join(home, "AppData", "Local");
    return path.join(base, "doubao-brain");
  }
  if (process.platform === "darwin") {
    return path.join(home, "Library", "Application Support", "doubao-brain");
  }
  const xdg = process.env.XDG_STATE_HOME || path.join(home, ".local", "state");
  return path.join(xdg, "doubao-brain");
}
